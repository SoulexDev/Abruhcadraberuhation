...
public class ThumbManager : MonoBehaviour
{
    ...
    public List<GameObject> captureObjects;
    ...
    public void GeneratePng(bool isPng = true, int i = 0)
    {
        try
        {
            ...
            for (int f = 0; f < captureObjects.Count; f++)
            {
                captureObjects.ForEach(c=>c.SetActive(false));
                captureObjects[f].SetActive(true);
                string filename = isPng ? GetFileName($"{filePrefix}{captureObjects[f].name}{fileSuffix}", "_Png", "png", (int)ResolutionWidth, (int)ResolutionHeight) : GetTempFileName((int)ResolutionWidth, (int)ResolutionHeight, i);
